...
import {
  ...
  FacilityUserApproveDetachResponse, FacilityDetailSuccessResponse, FacilityDetailErrorResponse,
  BenchmarkSuccessResponse, BenchmarkErrorResponse,
...
export async function getAllFederations(): Promise<FacilityResponse> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/proxy/federations/`);
  return res.json();
}
...
export async function updateFacilityYearlyData(facilityId: number, yearlyDataId: number, data: FacilityDetailFormData): Promise<FacilityDetailUpdateResponse> {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/proxy//facilities/${facilityId}/detail/${yearlyDataId}`, {
    method: "PUT",
    body: JSON.stringify(data),
    headers: {
      "content-type": "application/json"
